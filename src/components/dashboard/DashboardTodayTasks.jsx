...
export function DashboardTodayTasks({ tasks = [], users = [] }) {
  ...
  // Filter tasks due today (in local timezone)
  ...
  const displayedTasks = todayTasks.slice(0, 5);
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          {todayTasks.length > 5 && (
            <button
              type="button"
              onClick={() => navigate('/tasks?preset=today')}
            ...
            <table className="w-full text-left text-[12.5px]">
              ...
              <tbody className="divide-y divide-[#F4F4F5]">
                {displayedTasks.map((task) => {
                  const assigneeIds = Array.isArray(task.assigned_to_ids) && task.assigned_to_ids.length > 0
